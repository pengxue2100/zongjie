...
55.	Docker集成了开发环境和生产环境
56.	Cookie和session的区别
（1）Cookie数据存放在客户端的浏览器上，session数据放在服务器上
（2）很多浏览器都限制一个站点最多保存20个Cookie，单个cookie保存的数据不能超过4k
（3）cookie并不是很安全，别人可以分析存放在本地的cookie并进行cookie欺骗，考虑到安全应该使用session
（4）可以将登陆信息等重要信息保存到session，其他信息如果要保留可以存放在cookie
总而言之，cookie在客户端保存数据，不安全。只能保存字符串，且是少量字符串。
Session在服务端保存数据，安全，可以保存对象数据，数据无限制。
（5）默认情况下，浏览器关闭后，cookie数据被销毁；
Session什么时候被销毁？
（1）	服务器关闭
（2）	Session对象调用invalidate（）
（3）	Session默认失效时间是30分钟
57.	http请求中的get方法和post方法的区别
get一般用于查询和获取资源信息，指定的资源经服务器解析后返回响应内容，必要时，可以将查询字符串参数追加到URL末尾，以便将消息发送给服务器。请求参数不安全，提交数据量小。
post一般用于更新资源信息，通常会用来传输实体的本体，提交数据比较安全，提交数据无限制。
58.	转发和重定向的区别
（1）重定向是浏览器发送请求并收到响应后再次向一个新地址发请求，转发是服务器收到响应后为了完成响应转到另一个资源
（2）重定向中有两次请求对象，不共享数据；转发只产生一次请求对象且在组件间共享数据
（3）重定向后地址栏地址改变；转发则不会
（4）重定向的新地址可以是任意地址；转发必须是统一应用内的某个资源
转发的方法：
（1）	通过HttpServletRequet的getRequestDispatcher()方法获得
（2）	通过ServletContext的getRequestDispatcher()方法获得
重定向的方法：
HttpServletResponse的sendRedirect()方法
重定向可以实现图片的异步上传。
一般访问web-inf下的文件只能通过转发来实现
59.	spring中Bean的作用域有哪些？
（1）singleton（单例模式），使用该属性定义Bean时，IOC容器仅创建一个Bean实例，
IOC容器每次返回的是同一个Bean实例
（2）prototype（原型模式），使用该属性定义Bean时，IOC容器可以创建多个实例，每次返回的都是一个新实例
（3）request（HTTP请求），该属性仅对Http请求产生作用，使用该属性定义Bean时，每次http请求都会创建一个新的bean，适用于WebApplicationContext环境
（4）session（会话），该属性仅适用于http session，同一个session 共享一个bean实例，不同session使用不同的实例
60.spring框架实现实例化和依赖注入的方式分别是什么？
（1）使用构造器实例化Bean。SpringIOC容器能使用有参构造器也能使用无参构造器两种方式创建bean
（2）使用静态工厂方式实例化bean。使用这种方式除了指定必须的class属性，还要指定factory-method属性来指定实例化Bean方法，该方法必须是静态方法。使用静态工厂方法也允许指定方法参数，spring IOC容器经调用此属性指定的方法来获取bean
（3）使用实例工厂方法实例化bean。使用这种方式不能指定class属性，此时必须使用factory-bean属性来指定工厂Bean的id，factory-method属性指定实例化bean的方法，使用实例工厂方法也允许指定方法参数，方式和使用构造器方法一样。
依赖注入的方式：
（1）	基于构造函数的注入
构造函数注入就是通过bean类的构造方法，将bean依赖的对象注入，构造函数的参数一般情况下就是依赖项，spring容器会根据bean中指定的构造函数参数来指定调用哪个构造。
...
（4）	基于注解的依赖注入
@Autowired注解
默认按类型装配，默认情况下必须要求依赖对象必须存在，如果要允许null值，可以设置他的required属性为false
@Resource注解
